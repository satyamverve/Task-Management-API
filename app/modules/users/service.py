...
from Final_Demo.app.models.db_models import User
import Final_Demo.app.dto.user_schemas as user_schemas
...
def add_user(db: Session, user: user_schemas.UserSignUp):
...
def update_user(db: Session, email: str, user_update: user_schemas.UserUpdate):
    user = db.query(User).filter(User.email == email).first()
...
def user_change_password(db: Session, email: str, user_change_password_body: user_schemas.UserChangePassword):
    user = db.query(User).filter(User.email == email).first()
...
def update_me(db: Session, email: str, user_update: user_schemas.UserUpdateMe):
    user = db.query(User).filter(User.email == email).first()
